    ...
    pub fn handle_alchemy_input(&mut self, action: Action) {
        match action {
            ...
            Action::MenuDown => {
                let player_id = self
                    .get_player_id()
                    .expect("Player not found during alchemy browsing");
                let count = self
                    .world
                    .query::<(&InBackpack,)>()
                    .iter()
            ...
            Action::MenuSelect => {
                let player_id = self
                    .get_player_id()
                    .expect("Player not found during alchemy selection");
                    ...
                    if self.alchemy_selection.contains(&id) {
                        ...
                        if self.alchemy_selection.len() == 1 {
                                ...
                                || (n1 == "Potion of Speed" && n2 == "Potion of Strength")
                            {
                                self.world
                                    .despawn(item1)
                                    .expect("Failed to despawn alchemy item 1");
                                self.world
                                    .despawn(item2)
                                    .expect("Failed to despawn alchemy item 2");
