...
impl App {
    pub fn generate_level(&mut self, player_snapshot: Option<EntitySnapshot>) {
        ...
        if let Some(snapshot) = player_snapshot {
            ...
            for item_name in starting_items {
                if let Some(item_raw) = self.content.items.iter().find(|i| i.name == item_name).cloned() {
                    ...
                    if item_name == "Dagger" || item_name == "Leather Armor" { self.equip_item(item_id); }
                }
            }
        }
        ...
        let branch_str = match self.current_branch { Branch::Main => "Main", Branch::Gardens => "Gardens", Branch::Vaults => "Vaults" };
        let available_items: Vec<&crate::content::RawItem> = self.content.items.iter()
            .filter(|i| self.dungeon_level >= i.min_floor && self.dungeon_level <= i.max_floor)
            .filter(|i| i.branches.as_ref().map_or(true, |b| b.contains(&branch_str.to_string())))
            .collect();
        ...
        if mb.rooms.len() > 1 && !available_items.is_empty() {
            ...
            for _ in 0..3 {
                ...
                for item in &available_items {
                    if roll < item.spawn_chance { selected_item = item; break; }
        ...
        for pos in &mb.door_spawns { crate::spawner::spawn_door(&mut self.world, pos.0, pos.1); }
        for pos in &mb.trap_spawns {
            if self.current_branch == Branch::Gardens { crate::spawner::spawn_spore(&mut self.world, pos.0, pos.1); }
            else { crate::spawner::spawn_trap(&mut self.world, pos.0, pos.1); }
        }

        let available_monsters: Vec<&crate::content::RawMonster> = self.content.monsters.iter()
            .filter(|m| self.dungeon_level >= m.min_floor && self.dungeon_level <= m.max_floor)
            .filter(|m| m.branches.as_ref().map_or(true, |b| b.contains(&branch_str.to_string())))
            .collect();

        let mut monster_spawns = mb.monster_spawns.clone();
        if self.escaping { monster_spawns.extend(mb.monster_spawns.clone()); }
        ...
        for spawn in &monster_spawns {
            ...
            for m in &available_monsters {
                if roll < m.spawn_chance { selected_monster = m; break; }
                roll -= m.spawn_chance;
            }
            crate::spawner::spawn_monster(&mut self.world, spawn.0, spawn.1, selected_monster, self.dungeon_level);
        }

        if let Some(spawn) = mb.boss_spawn {
            if let Some(raw) = self.content.monsters.iter().find(|m| m.is_boss == Some(true) && m.min_floor == self.dungeon_level) {
        ...
        if self.dungeon_level == 10 && !self.escaping {
            if let Some(amulet) = self.content.items.iter().find(|i| i.name == "Amulet of the Ancients") {
