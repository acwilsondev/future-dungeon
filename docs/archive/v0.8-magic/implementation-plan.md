...
- Expand status effect components to support new magic-related effects (e.g., `Armored`, `Warped`).
...
- Interaction flow:
    1. If `ShrineTried` is set, display `"The shrine is silent. Peace be with you."` and stop.
    2. If the player's total max mana is already 5, fail with a message.
    3. Otherwise, prompt the player to attempt (confirm/decline). If the player declines, do nothing.
    4. On confirm, roll CHA check against DC = `10 + total maximum mana (all colors)`.
    5. On failure: display a failure message and set `ShrineTried`.
    6. On success: increment `max_orange` or `max_purple` (matching shrine color) by 1 and set `ShrineTried`.
